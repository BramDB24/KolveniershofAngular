...
export class VierweeksePlanningComponent implements OnInit {
        ...
        this.route.data.subscribe(
            ...
            err => {
                this.newTemplateError =
                    'Er liep iets fout bij het ophalen van de templates, probeer opnieuw.';
            }
        );
    ...
    public createTemplate(naam: string) {
        if (
            !naam ||
            naam == '' ||
            this.templates.map(t => t.naam).includes(naam)
        ) {
            this.newTemplateError =
                'Er is al een template met de ingegeven naam!';
        } else {
            this._templateService.postTemplate(naam).subscribe(
                val => {
                ...
                err => {
                    this.newTemplateError =
                        'Er liep iets fout bij het creëren van de dagplanning, probeer opnieuw.';
                }
            );
        }
    }
    ...
    public deleteTemplate() {
        if (
            !this.huidigeTemplate.isActief &&
            confirm(
                "Bent u zeker dat u template '" +
                    this.huidigeTemplate.naam +
                    "' wilt verwijderen?"
            )
        )
            this._templateService
                .deleteTemplate(this.huidigeTemplate.id)
                .subscribe(
                    val => {
                        alert(
                            'template ' + val.naam + ' is succesvol verwijderd!'
                        );
                    },
                    err => {
                        this.errorMessage =
                            'Template kon niet verwijderd worden!';
