...
export class HomepageEditAtelierComponent implements OnInit, OnChanges {
  ...
  private initialiseerFormGroup() {
    this.dagAtelierFormGroup = this.fb.group({
      atelierNaam: [
        this.dagAtelier ? this.dagAtelier.atelier.naam : '',
  ...
  public verwijderGebruiker(gebruiker: Gebruiker): void {
    ...
    const index = this.dagAtelierCopy.gebruikers.findIndex(
      entry =>
  ...
  public getNietAanwezigen(): Array<Gebruiker> {
    return this.alleGebruikers
      .filter(
        gebruiker =>
          this.dagAtelierCopy.gebruikers.filter(
            aanwezige =>
  ...
  public onSubmit(): void {
    ...
    if (!this.dagAtelier || this.dagAtelier.atelier.naam !== formAtelierNaam) {
      this.dagAtelierCopy.atelier = this.ateliers.find(
  ...
  public voegGebruikerToeAanCopy(): void {
    ...
    const teZoekenGebruiker = (document.getElementById(
      'gebruikerToevoegenLijst'
    ) as HTMLInputElement).value;
    const gevondenGebruiker = this.alleGebruikers.find(
      gebruiker =>
    ...
    if (
      this.dagAtelierCopy.gebruikers.find(
        aanwezige =>
          aanwezige.voornaam === gevondenGebruiker.voornaam &&
          aanwezige.achternaam === gevondenGebruiker.achternaam
      )
    ) {
